...
LDFLAGS := $(shell echo "-X 'archive.org/version.Version=`git describe --tags --abbrev=0`'")
LDFLAGS := $(shell echo "${LDFLAGS} -X 'archive.org/version.Commit=`git rev-parse --short HEAD`'")
LDFLAGS := $(shell echo "${LDFLAGS} -X 'archive.org/version.BuildDate=`date +%FT%T%z`'")
...
GOFILES ?= $(wildcard ./cmd/archive.org/*.go)
PROJECT := github.com/wabarc/archive.org
